...
int main(int argc, char **argv) {
    ...
    Matrix3d rotation_matrix = Matrix3d::Identity(); // initialize 3x3 matrix with identity matrix

    // The rotation vector uses AngleAxis, the underlying layer is not directly Matrix, but the operation can be treated as a matrix
    ...
    rotation_matrix = rotation_vector.toRotationMatrix(); // this calls the Rodrigues Roration formula

    // coordinate transformation with AngleAxis
    Vector3d v (1, 0, 0);
    Vector3d v_rotated = rotation_vector * v;
    cout << "(1, 0, 0) after rotation (by AngleAxis) = " << v_rotated.transpose() <<endl;
    ...
    Vector3d v_transformed = T * v; // equivalent to R*x+t
